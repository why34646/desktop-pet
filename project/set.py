...
class ResizeDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.resize_cmd_file = get_app_root() / ".resize_cmd"
...
class SettingsDialog(QDialog):
    ...
    def open_memory_manager(self):
        """打开记忆管理窗口"""
        try:
            from talk.memory_manager import MemoryManagerDialog
            dialog = MemoryManagerDialog(self)
            dialog.exec_()
        except Exception as e:
            QMessageBox.critical(self, "错误", f"无法打开记忆管理: {str(e)}")
    ...
    def load_console_status(self):
        """加载显示运行框设置"""
        console_file = get_app_root() / ".console_show"
    ...
    def save_settings(self):
        try:
            ...
            if self.autostart_checkbox.isChecked():
                # 如果是 exe 模式: 直接用 sys.executable(exe 路径)
                # 如果是开发模式: 用 python + start.py
                if getattr(sys, "frozen", False):
                    command = f'"{sys.executable}"'
                else:
                    script_path = str(get_app_root() / "start.py")
